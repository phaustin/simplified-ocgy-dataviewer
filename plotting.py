...
def update_legend(fig, cruise, hov_station, click_stations):
    ...
    if cruise == "GIPY0405":
        fig["layout"]["legend"]["title"]["text"] = (
            "<b>" + "GIPY04 & GIPY05" + "</b>" + "<br></br>Selected Stations:"
        )
    else:
        fig["layout"]["legend"]["title"]["text"] = (
            "<b>" + str(cruise) + "</b>" + "<br></br>Selected Stations:"
        )
    return fig
        ...
        for i in range(8):
            if i < len(click_stations):
                ...
                click_xvals_temp, click_yvals_temp = get_x_y_values(
                    cruise, click_stations[i].lat, click_stations[i].lon, "Temperature"
                )
                fig["data"][i + 1].update(
                    x=click_xvals_temp,
                    y=click_yvals_temp,
...
def initialize_map(cruise):
    ...
    if cruise == "GIPY0405":
        fig.update_layout(
            margin={"r": 0, "t": 40, "l": 0, "b": 0}, title="GIPY04 and GIPY05"
        )
    else:
